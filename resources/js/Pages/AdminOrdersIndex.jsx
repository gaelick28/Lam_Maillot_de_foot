              ...
              <div className="flex gap-2">
                ...
                {(search || statusFilter) && (
                  <button
                    ...
                    className="flex-1 sm:flex-none px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors text-sm sm:text-base"
                  >
                    Réinitialiser
                  </button>
                )}
  ...
  <div className="overflow-x-auto">
    <table className="w-full">
      <thead className="bg-gray-50">
        <tr>
          <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">N° Commande</th>
          ...
          <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Montant</th>
          <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Statut</th>
          <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden lg:table-cell">Paiement</th>
      ...
      <tbody className="divide-y divide-gray-200">
        {orders.data.length > 0 ? (
          orders.data.map((order) => (
            <tr key={order.id} className="hover:bg-gray-50">
              ...
              <td className="px-3 sm:px-6 py-4">
                <Link href={`/admin/orders/${order.id}`} className="px-3 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium hover:bg-blue-200 transition-colors">
                  Détails
                </Link>
              </td>
            </tr>
        ...
        {/* Pagination */}
        {orders.last_page > 1 && (
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 bg-white rounded-lg shadow px-4 sm:px-6 py-4">
            <p className="text-xs sm:text-sm text-gray-600">
              {orders.from} à {orders.to} sur {orders.total} commandes
            </p>
            <div className="flex flex-wrap gap-1 sm:gap-2 justify-center">
              {orders.links.map((link, index) => (
                <Link key={index} href={link.url || '#'} className={`px-2 sm:px-3 py-1 rounded text-xs sm:text-sm ${
                  link.active ? 'bg-blue-600 text-white'
                    : link.url ? 'bg-gray-100 text-gray-700 hover:bg-gray-200'
