...
class CartController extends Controller
{
    ...
    public function show()
    {
        ...
        foreach ($sessionCart as $sessionItem) {
            ...
            $supplement = $this->pricing->calculateSupplement($sessionItem['nom'] ?? null, $sessionItem['numero'] ?? null, $patches);
            $total      = ($price + $supplement) * $quantity;

            $cartItems->push([
                'id'               => $sessionItem['id'],
                'club_name'        => $maillot->club->name ?? 'Club inconnu',
                'maillot_name'     => $maillot->nom ?? 'Maillot',
                'maillot_id'       => $sessionItem['maillot_id'],
                'image'            => $maillot->image,
                'size'             => $sessionItem['size'],
                'is_session'       => true,
        ...
        if (Auth::check()) {
            ...
            foreach ($cart->items as $item) {
                ...
                $supplement = $this->pricing->calculateSupplement($item->nom, $item->numero, $patches);
